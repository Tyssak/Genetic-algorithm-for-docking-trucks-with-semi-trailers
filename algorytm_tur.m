%% algorytm genetyczny stosujący metodę selekcji turniejową
...
pturniej = 1;  % prawdopodobieństwo w selekcji turniejowej (dla 1 - turniej deterministyczny)
tur = ceil(M/40); % wielkość turniejów
if (tur < 2)
    tur = 2;
end
...
if (opcja == 7)
    ...
    PSC(1,1,:) = [0 27];  %1 grupa punktów (połączonych, gdzie 1 się łączy z 2, 2 z 3 itd) [0,8]
    PSC(1,2,:) = [15 27]; %2 punkt z pierwszej grupy
    PSC(1,3,:) = [15 0];
    PSC(2,1,:) = [0 32];
    PSC(2,2,:) = [15 32];
    PSC(2,3,:) = [15 70];
...
elseif (opcja == 3)
    ...
    %Współrzędne ścian
    PSC(2,2,:)=[51 15]; %punkty symbolizujące krawędź parkingu
    PSC(2,1,:)=[51 0];  %od 11 do 12 i od 21 do 22
    PSC(1,2,:)=[56 15];
    PSC(1,1,:)=[56 0];
    PSC(2,3,:)=[0 15];
    PSC(1,3,:)=[70 15];
...
elseif (opcja == 1)
    ...
    PSC(1,1,:) = [0 8];  %1 grupa punktów (połączonych, gdzie 1 się łączy z 2, 2 z 3 itd) [0,8]
    PSC(1,2,:) = [15 8]; %2 punkt z pierwszej grupy
    PSC(1,3,:) = [15 0];
    PSC(2,1,:) = [0 12];
    PSC(2,2,:) = [15 12];
    PSC(2,3,:) = [15 70];
...
elseif(opcja == 2)
    ...
    %Współrzędne ścian
    PSC(2,2,:)=[40 20]; %punkty symbolizujące krawędź parkingu
    PSC(2,1,:)=[40 0];  %od 11 do 12 i od 21 do 22
    PSC(1,2,:)=[45 20];
    PSC(1,1,:)=[45 0];
    PSC(2,3,:)=[0 20];
    PSC(1,3,:)=[70 20];
elseif(opcja == 8)
    ...
    %Współrzędne ścian
    PSC(1,2,:) = [10 8]; %punkty symbolizujące krawędź parkingu
    PSC(1,1,:) = [0 8];  %od 11 do 12 i od 21 do 22
    PSC(2,2,:) = [10 13];
    PSC(2,1,:) = [0 13];
    PSC(1,3,:) = [10 0];
    PSC(2,3,:) = [10 70];
elseif(opcja == 4)
    ...
    %Współrzędne ścian
    PSC(1,2,:) = [40 55]; %punkty symbolizujące krawędź parkingu
    PSC(1,1,:) = [40 70];  %od 11 do 12 i od 21 do 22
    PSC(2,2,:) = [45 55];
    PSC(2,1,:) = [45 70];
    PSC(1,3,:) = [0 55];
    PSC(2,3,:) = [70 55];
elseif(opcja == 6)
    ...
    %Współrzędne ścian
    PSC(1,2,:) = [12 8]; %punkty symbolizujące krawędź parkingu
    PSC(1,1,:) = [0 8];  %od 11 do 12 i od 21 do 22
    PSC(2,2,:) = [12 12];
    PSC(2,1,:) = [0 12];
    PSC(1,3,:) = [12 0];
...
elseif(opcja == 5)
    ...
    %Współrzędne ścian
    PSC(2,2,:) = [7 10]; %punkty symbolizujące krawędź parkingu
    PSC(2,1,:) = [0 3];  %od 11 do 12 i od 21 do 22
    PSC(1,2,:) = [10 7];
    PSC(1,1,:) = [3 0];
    PSC(2,3,:) = [7 70];
    PSC(1,3,:) = [70 7];
elseif(opcja == 9)
    ...
    %Współrzędne ścian
    PSC(2,2,:) = [38 35]; %punkty symbolizujące krawędź parkingu
    PSC(2,1,:) = [50 47];  %od 11 do 12 i od 21 do 22
    PSC(1,2,:) = [35 38];
    PSC(1,1,:) = [47 50];
    PSC(2,3,:) = [38 0];
    PSC(1,3,:) = [0 38];
...
while koniec == false
    ...
    disp('Proszę czekać');
    % wylicz liczbę kroków (długość chromosomu)
    ...
    index_n = zeros(1,M);
    % funkcje przystosowania chromosomów wylosowanych w turnieju
    f_tur = zeros(1,tur);
    ...
    % zamień na reprezentację binarną (nieparzyste we i we_n - u(t), parzyste - r(t)
    h = 1;
    for i=1:N
        for j=1:M
            if(ini(i,j)==0)
    ...
    while petla
        ...
        if k > 0
            ...
            i = 1;
            while i<=M
                % losuj osobniki do turnieju (bez powtórzeń)
                los_tur = randperm(M,tur);
                for wii=1:tur
                    f_tur(wii) = wyborDec(los_tur(wii));
                end
                [f_tur_Inc,id_tur] = sort(f_tur);

                % sprawdź, czy należy przeprowadzić turniej
                if (rand < pturniej)
                    % jeśli 2 najlepsze osobniki mają równe funkcje celu,
                    % to wybierz spośród nich losowo
                    if(f_tur_Inc(tur)==f_tur_Inc(tur-1))
                        if (randi([0 1]) == 0)
                            index_n(i) = index(los_tur(id_tur(tur)));
                        else
                            index_n(i) = index(los_tur(id_tur(tur-1)));
                        end
                    else
                        % weź najlepszego osobnika z turnieju
                        index_n(i) = index(los_tur(id_tur(tur)));
                    end
                else
                    % jak nie było turnieju, to wybierz losowego osobnika z
                    % turnieju
                    index_n(i) = index(los_tur(id_tur(ceil(5*rand+1))));
                end
                i = i + 1;
            end
    ...
    if(pokaz)
        wyswietl(0,u0,karaOverall(index(1)),pk ,pm ,M ,MAX_Pokolen,przegrana, u_zm, r_zm,NajWybor, ...
